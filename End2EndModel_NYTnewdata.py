...
def test_model(nn_model,testdata,index2word,resultfile=''):
    ...
    P, R, F,pr,p,r = evaluavtion_triple(testresult)
#    print P, R, F
    return P, R, F,pr,p,r


def train_e2e_model(eelstmfile, modelfile,resultdir,npochos,
                    lossnum=1,batch_size = 100,retrain=False):
    ...
    while (epoch < npochos):
        ...
        for x, y in get_training_batch_xy_bias(x_train, y_train, max_s, max_s,
            ...
            if epoch > saveepoch:
                ...
                P, R, F,pr,p,r = test_model(nn_model, testdata, target_idex_word,resultfile)
                if F > maxF:
                    maxF=F
                    save_model(nn_model, modelfile)
                print ' epoch %d: P=%f R=%f F=%f pr=%d p=%d r=%d' %(epoch,P, R, F,pr,p,r)
                PRF.append([P,R,F,pr,p,r])
        print('epoch: '+str(epoch)+', time: '+str(time.time()-t)+'s')
    cPickle.dump(PRF,open('./NYTprf.pkl','wb'))
    return PRF
...
if __name__=="__main__":
    ...
    e2edatafile = "./data/demo/e2edata.pkl"
    modelfile = "./data/demo/model/"
    ...
    if not os.path.exists(modelfile+"e2e_lstmb_model.pkl"):
        print "Lstm data has extisted: "+e2edatafile
        print "Training EE model...."
        train_e2e_model(e2edatafile, modelfile+"e2e_lstmb_model.pkl",resultdir,
